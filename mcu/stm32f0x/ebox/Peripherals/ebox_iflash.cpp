 ...
 *@retval   E_FlashStates
*/
E_Flash::E_Flash(uint8_t startAddr,uint8_t nPage){
	// 初始化flash起始,结束地址
...
int E_Flash::write(uint32_t offsetAdd, uint8_t *buf, uint32_t iNbrToWrite)
{
	...
	if ((_start_addr == 0)||(iAddress == _end_addr)){
		DBG("write: flash 定义错误，请检查起始地址和页数量\r\n");
		return 0;
	}
...
*/
int E_Flash::read(uint32_t offsetAdd, uint8_t *buf, uint32_t iNbrToRead)
{
	// 绝对地址
	uint32_t iAddress = _start_addr + offsetAdd;

	if (_start_addr == 0){
		DBG("read: flash 定义错误，请检查起始地址和页数量\r\n");
		return 0;
	}
