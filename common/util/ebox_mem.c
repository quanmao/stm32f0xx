// 1 不能同时使用O3 & Otime优化，会出错。如果同时选择O3,OTime,Ospace优化则可以
// 2 O3 or OTime选择一个优化项，可以正常运行
...
static void insert_block_into_freeList( eboxBlockLink_t *pxBlockToInsert)
{
  ...
  to itself. */
  /*最后，要是pxBlockToInsert没有和pxIterator合并，则还要修改pxIterator的Next指针，这样整条链表才完整无误*/
